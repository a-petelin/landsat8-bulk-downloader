...
for ent in $RESULT; do curl -c login.cook -b login.cook -L 'https://earthexplorer.usgs.gov/order/addbulkscene?scenes='$ent'&collection_id=12864&originator=INVSVC' 2> /dev/null | jq -C; done
DATA=$(curl 'https://earthexplorer.usgs.gov/inventory/json/v/1.4.0/logout?jsonRequest=%7b%22apiKey%22:%22'$APIKEY'%22%7d')
echo 'DATA=' $DATA
TEST=$(curl 'https://api.ghostinspector.com/v1/tests/5e4861bca86a49794d824fd5/execute/?apiKey=08be0ee1f51f733da0d300dbcead9bb7edaff265')
echo 'TEST=' $(echo $TEST | jq -r ".data.passing")
